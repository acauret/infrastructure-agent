...
async def run():
    ...
    try:
        ...
        print(f"Warning: GitHub MCP Server initialization failed: {e}")
        print("Continuing with Azure MCP Server only...")
    
    # Create sessions for both servers and keep them active throughout the conversation
    if github_mcp_server:
        async with azure_mcp_server.create_session(), github_mcp_server.create_session():
            ...
            # Display available tools from GitHub MCP
            github_tools = github_mcp_server.list_tools()
            print(f"\nLoaded {len(github_tools)} GitHub MCP tools:")
            for tool_name, tool_desc in github_tools:
                short_desc = tool_desc.split('.')[0] if '.' in tool_desc else tool_desc[:60]
                print(f"  • {tool_name}: {short_desc}")

            # Combine tools from both servers
            available_tools = azure_mcp_server.formatted_tools + github_mcp_server.formatted_tools
            print(f"\nTotal tools available: {len(available_tools)}")

            # Start conversational loop
            await run_conversation_loop(client, azure_tools, github_tools, available_tools, azure_mcp_server, github_mcp_server)
    else:
        # Only Azure MCP server available
        async with azure_mcp_server.create_session():
            ...
            # Only Azure tools available
            available_tools = azure_mcp_server.formatted_tools
            print(f"\nTotal tools available: {len(available_tools)}")

            # Start conversational loop
            await run_conversation_loop(client, azure_tools, [], available_tools, azure_mcp_server, None)


async def run_conversation_loop(client, azure_tools, github_tools, available_tools, azure_mcp_server, github_mcp_server):
            ...
            if response_dict.get("tool_calls"):
                for tool_call in response_dict["tool_calls"]:
                    try:
                        ...
                        if function_name in [tool[0] for tool in azure_tools]:
                            content = await azure_mcp_server.call_tool(function_name, function_args)
                        elif github_mcp_server and function_name in [tool[0] for tool in github_tools]:
                            content = await github_mcp_server.call_tool(function_name, function_args)
                        else:
                            raise Exception(f"Tool {function_name} not found in any available MCP server")
