...
if not AZURE_API_KEY:
    raise ValueError("AZURE_OPENAI_API_KEY or AZURE_OPENAI_KEY environment variable is not set")

# Convert OpenAI endpoint to Cognitive Services endpoint format for Azure AI Inference
def get_inference_endpoint(base_endpoint: str, model: str) -> str:
    """Convert OpenAI endpoint to Azure AI Inference endpoint format"""
...
class DynamicToolManager:
    ...
    def _extract_message_content(self, message: Dict) -> str:
        """Extract text content from Azure AI Inference message format"""
...
def convert_messages_to_azure_ai_format(messages: List[Dict]) -> List:
    """Convert OpenAI-style messages to Azure AI Inference format"""
    converted_messages = []
    ...
    for msg in messages:
        if msg["role"] == "system":
            ...
            if msg.get("tool_calls"):
                # For Azure AI Inference, include the content AND tool calls
                content = msg.get("content") or "I'll help you with that."
                # Azure AI Inference needs the assistant message to be properly formatted
                converted_messages.append(AssistantMessage(content=content))
            else:
                converted_messages.append(AssistantMessage(content=msg["content"]))
        elif msg["role"] == "tool":
            # Only include tool messages if there was a preceding assistant message with tool calls
            # Azure AI Inference is strict about this
            if converted_messages and isinstance(converted_messages[-1], AssistantMessage):
                converted_messages.append(ToolMessage(
                    content=msg["content"],
                    tool_call_id=msg.get("tool_call_id", "")
                ))
            # Skip tool messages that don't have proper context
...
async def run_conversation_loop(client: ChatCompletionsClient, tool_manager: DynamicToolManager):
    ...
    while True:
        try:
            ...
            # Streaming-only API call with tool configuration
            ...
            # Process the streaming response - simplified for Azure AI Inference
            ...
            for chunk in response:
                if chunk.choices and chunk.choices[0].delta:
                    ...
                    # Handle tool call streaming - Azure AI Inference simplified format
            ...
            if not assistant_message["content"] and assistant_message["tool_calls"]:
                assistant_message["content"] = None
            
            if not assistant_message["tool_calls"] or not any(tc["function"]["name"] for tc in assistant_message["tool_calls"]):
            ...
            if assistant_message.get("tool_calls"):
                ...
                # Get the final response from the model after tool calls - streaming only
