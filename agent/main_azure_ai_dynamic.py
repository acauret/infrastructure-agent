...
TOOL_CATEGORIES = {
    "azure": {
        "keywords": [
            ...
            "load testing", "grafana", "datadog", "marketplace", "network", "vnet", "subnet"
        ],
        "tools": [
    ...
    "github": {
        "keywords": [
            ...
            "search code", "organization", "user", "notification", "clone",
            "push", "merge", "review", "copilot", "git"
        ],
        "tools": [
...
def convert_messages_to_azure_ai_format(messages: List[Dict]) -> List:
    """Convert OpenAI-style messages to Azure AI Inference format with strict validation"""
    converted_messages = []
    ...
    for msg in messages:
        if msg["role"] == "system":
            ...
            # Check if this is an Azure AI message (with tool calls)
            if msg.get("azure_message"):
                converted_messages.append(msg["azure_message"])
            else:
            ...
            # Add tool message using Azure AI format
...
async def run_conversation_loop(client: ChatCompletionsClient, tool_manager: DynamicToolManager):
    ...
    # Load system prompt
    system_prompt_path = os.path.join(os.path.dirname(__file__), "..", "prompts", "system_prompt_azure_ai.txt")
    try:
        with open(system_prompt_path, 'r', encoding='utf-8') as f:
            system_prompt = f.read().strip()
    except FileNotFoundError:
        system_prompt = "You are an Azure infrastructure expert assistant with access to Azure and GitHub tools for comprehensive DevOps and infrastructure management."
    ...
    print("🚀 Using Azure AI Inference Client for improved performance")
    ...
    while True:
        try:
            ...
            # Debug: Print message structure (only for troubleshooting)
            if logger.isEnabledFor(logging.DEBUG):
                print(f"🔍 Converting {len(messages)} messages to {len(azure_messages)} Azure AI messages")
                for i, msg in enumerate(azure_messages):
                    print(f"  {i}: {type(msg).__name__}")
            
            # Streaming API call with tool configuration
            ...
            for chunk in response:
                if chunk.choices and chunk.choices[0].delta:
                    ...
                    # Handle tool call streaming - Azure AI Inference format
                    if hasattr(delta, 'tool_calls') and delta.tool_calls:
                        for tool_call in delta.tool_calls:
                            # Initialize tool call on first encounter
                            ...
                            if hasattr(tool_call, 'function') and tool_call.function:
                                ...
                                # Get function name (usually in first chunk)
            ...
            if assistant_message.get("tool_calls") and any(tc["function"]["name"] for tc in assistant_message["tool_calls"]):
                ...
                for tc in assistant_message["tool_calls"]:
                    if tc["function"]["name"]:  # Only include valid tool calls
                        azure_tool_call = ChatCompletionsToolCall(
                            id=tc["id"],
                        ...
                        azure_tool_calls.append(azure_tool_call)
                
                # Add assistant message with tool calls using Azure AI format
                azure_assistant_msg = AssistantMessage(tool_calls=azure_tool_calls)
                messages.append({"role": "assistant", "azure_message": azure_assistant_msg, "tool_calls": assistant_message["tool_calls"]})
            else:
            ...
            if assistant_message.get("tool_calls") and any(tc["function"]["name"] for tc in assistant_message["tool_calls"]):
                ...
                azure_messages_with_tools = convert_messages_to_azure_ai_format(messages)
                
                final_response = client.complete(
                    model=AZURE_OPENAI_MODEL,
                    messages=azure_messages_with_tools,
                    tools=available_tools,
                    stream=True,
                )
...
async def run():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        ...
        print("✅ Azure AI Inference Dynamic Agent initialized successfully!")
        print("💡 Use 'python main_azure_ai_dynamic.py' for interactive mode")
